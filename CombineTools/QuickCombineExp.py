...
#options="--testStat LHC --significance --saveToys --saveHybridResult -T 100 -i 5 -S 1"
#Exp. Limit:
options=" -v 0 --saveToys --saveHybridResult -T 250 -i 3 "
#for j in range(0,1):
...
for j in range(0, 40):
	...
	for i in range (0,4):
		...
		f.write("set SCRAM_ARCH=slc6_amd64_gcc481\n")
        	...
        	f.write("cd Sens3/ \n")
        	f.write("%s %s %s -s %d -n DummyTestBin%dCombine%d --singlePoint %2.2f \n" %(basecommand, datacard, options, r,j,i, point))
        	f.close()
        	os.system("chmod 744 %sBin%dbsub%d.sh" %(sys.argv[1],j,i))
		os.system("bsub -R \"pool>30000\" -q 1nh -J job%d < %sBin%dbsub%d.sh" %(i,sys.argv[1],j,i))

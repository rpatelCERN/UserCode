...
void MakeDataCardsMerged(int mGlu, int MLSP, int lumi=4, TString Options="",  bool SingleBin=false, int Limitbin=0){
                ...
                for ( int hti=1; hti<=3; hti++ ) {
                   ...
                   // if(ij>1)continue;//have merged jet bins
                    if(mhti==3){
                        ...
                        float ttsum=0;
                       // for(int j=1; j<=3; ++j){
                            sig4t1500Sum=sig4t1500_[nb]->GetBinContent( ij, mhti, 2 )+sig4t1500_[nb]->GetBinContent( ij, mhti, 3 )+sig4t1500Sum;
                        sig4t1200Sum=sig4t1200_[nb]->GetBinContent( ij, mhti, 2 )+sig4t1200_[nb]->GetBinContent( ij, mhti, 3 )+sig4t1200Sum;
                        sig4q1400Sum=sig4q1400_[nb]->GetBinContent( ij, mhti, 2 )+sig4q1400_[nb]->GetBinContent( ij, mhti, 3 )+sig4q1400Sum;
                       sig4q1000Sum=sig4q1000_[nb]->GetBinContent( ij, mhti, 2 )+sig4q1000_[nb]->GetBinContent( ij, mhti, 3 )+sig4q1000Sum;
                        sig4b1500Sum=sig4b1500_[nb]->GetBinContent( ij, mhti, 2 )+sig4b1500_[nb]->GetBinContent( ij, mhti, 3 )+sig4b1500Sum;
                        sig4b1000Sum=sig4b1000_[nb]->GetBinContent( ij, mhti, 2 )+sig4b1000_[nb]->GetBinContent( ij, mhti, 3 )+sig4b1000Sum;
                        
                        
                        qcdsum=qcd_[nb]->GetBinContent( ij, mhti, 2 )+qcd_[nb]->GetBinContent( ij, mhti, 3 )+qcdsum;
                        zi[ibin]=ZI_[nb]->GetBinContent( ij, mhti, 2 )+ZI_[nb]->GetBinContent( ij, mhti, 3 );
                        wsum=WJ_[nb]->GetBinContent( ij, mhti, 2 )+WJ_[nb]->GetBinContent( ij, mhti, 3 )+wsum;
                            ttsum=tt_[nb]->GetBinContent( ij, mhti, 2)+tt_[nb]->GetBinContent( ij, mhti, 3)+ttsum;
                    //    }
                    ...
                    if(mhti==2){
                        ...
                        if(hti==3){
                            if(nb==0){
                                //if(ij!=1)continue;
                                sig4t1500[ibin]=sig4t1500_[nb]->GetBinContent( ij, mhti, 3 )+sig4t1500_[nb]->GetBinContent( 2, mhti, 3 )+sig4t1500_[nb]->GetBinContent( 3, mhti, 3 );
...
//    return;
    ...
    if(SingleBin){
    TString str=(TString::Format("MergedBins/SensStudy2binLumi%d_Bin%d_M%d_m%d_%s.dat",lumi,Limitbin, mGlu, MLSP,Options.Data())).Data();
   // const char outputfilename[100]=str.Data();
    ...
    fprintf(fp, "imax %d number of channels\n", 4);//signal region PLUS 3 control bins
        ...
        //4 bins
        fprintf(fp, "\n observation %g %g %g %g", obs[Limitbin], qcdscale*qcd[Limitbin],wj[Limitbin],6.0*ttbar[Limitbin]);
   ...
   //    fprintf(fp, "\nrate %g %g %g %g %g  ", sig[Limitbin], qcd[Limitbin],zi[Limitbin],wj[Limitbin],ttbar[Limitbin]);
       ...
       if(ttbar[Limitbin]>0.0000000000000000001)fprintf(fp, " %g ",ttbar[Limitbin]);
        ...
        fprintf(fp, "rateBqcd%d lnU - %g - - - ",Limitbin, logUErr );
        fprintf(fp, " - %g - - - ",logUErr);
        fprintf(fp, " - - - - - ");
        fprintf(fp, " - - - - - \n");
        ...
        fprintf(fp, "rateBW%d lnU - - - %g - ",Limitbin, logUErr );
        fprintf(fp, " - - - - - ");
        fprintf(fp, " - - - %g - ",logUErr);
        fprintf(fp, " - - - - - \n");
        ...
        while(ttbar[Limitbin]/logUErr>0.1)logUErr=logUErr+(1.0*logUErr);

        fprintf(fp, "rateBttbar%d lnU - - - - %g ",Limitbin, logUErr );
        fprintf(fp, " - - - - - ");
        fprintf(fp, " - - - - - ");
        fprintf(fp, " - - - - %g \n",logUErr);
        ...
        int nbins=72;
        TString str=(TString::Format("MergedBins/SensStudySys_M%d_m%d_%s_Lumi%d.dat", mGlu, MLSP,Options.Data(), lumi));
      ...
      //  const char outputfilename[100]=str.Data();
        ...
        fprintf(fp, "\n observation ");
        for(int i=0; i<nbins; ++i)fprintf(fp, " %g %g %g %g ",obs[i], qcdscale*qcd[i], wj[i], 6*ttbar[i]);
        ...
        for(int i=0; i<nbins; ++i){
            ...
            if(ttbar[i]>0.0000000000000000001)fprintf(fp, " %g %g %g %g %g ",0.0, 0.0, 0.0,0.0,6*ttbar[i]);
            else fprintf(fp, " %g %g %g %g %g ",0.0, 0.0, 0.0,0.0,6.0);
...
sprintf(weightstring, "*(weight*2000)"); //bkg xsec weight
...
TFile*f0=new TFile(TString::Format("Bins4DBINS2fb.root").Data(), "RECREATE");
for(int b=0; b<nBinsBjets; ++b){

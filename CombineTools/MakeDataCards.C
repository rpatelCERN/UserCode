...
void MakeDataCards(int mGlu, int MLSP, int lumi=4, TString Options="",  bool SingleBin=false, int Limitbin=0){
...
///   const int totalbins=54;
    ...
    float mu=1; //change this to add signal to the observed data for tests (DEFAULT obs set to SM)
    /*
...
//    return;
    ...
    for(int i=0; i<totalbins; ++i){
      ...
      if(Options=="T1tttt" && mGlu==1200)sig[i]=sig4t1200[i];
        ...
        obs[i]=int(qcd[i]+ zi[i]+wj[i]+ttbar[i] + (mu*sig[i])+0.5);
    ...
    if(SingleBin){
    TString str=(TString::Format("datfiles/SensStudyLumi%d_Bin%d_M%d_m%d_%s.dat",1,Limitbin, mGlu, MLSP,Options.Data())).Data();
   // const char outputfilename[100]=str.Data();
    ...
    fprintf(fp, "imax %d number of channels\n", 1);
        ...
        fprintf(fp, "\n observation %d", obs[Limitbin]);
        

            fprintf(fp, "\nbin %d %d %d %d %d", Limitbin, Limitbin,Limitbin, Limitbin,Limitbin);
            fprintf(fp, "\nprocess sig QCD Zinv WJ ttbar");
            fprintf(fp, "\nprocess %d %d %d %d %d", 0, 1,2,3,4);
      //  if(qcd[Limitbin]<0.000001)qcd[Limitbin]=qcd[Limitbin]+0.001;
       // if(zi[Limitbin]<0.000001)zi[Limitbin]=zi[Limitbin]+0.001;
      //    if(wj[Limitbin]<0.000001)wj[Limitbin]=wj[Limitbin]+0.001;
      //  if(ttbar[Limitbin]<0.000001)ttbar[Limitbin]=ttbar[Limitbin]+0.001;
        ...
        float Q=sig[Limitbin]/sqrt(sig[Limitbin]+qcd[Limitbin]+ zi[Limitbin]+wj[Limitbin]+ttbar[Limitbin]);
            
               // float total=0;
        
               // for(int i=0; i<totalbins; ++i)total=total+(10*qcd[i]);
      //  if(Q>0.2){
        
        
                float qcdEC=int(qcd[Limitbin]*qcdscale+0.5);
                float alpha=1.0;
        
                alpha=1.0;
        if(qcdEC>0.0000000001){
            fprintf(fp, "deltaBqcd lnN");
            alpha=sqrt(qcdEC)/qcdEC+1;
             fprintf(fp, " - %g - - - \n",alpha);
        }
        
        else{
            fprintf(fp, "BqcdgmN gmN 0 - %g - - - \n", alpha);
        }
        
                 // total=0;
                alpha=1.0;
            // for(int i=0; i<totalbins; ++i)total=total+(wj[i]);
                int wEC=int(wj[Limitbin]*uncScale+0.5);
        if(wEC>0.0000000001){
            alpha=wj[Limitbin]/wEC;
            fprintf(fp, "deltaBW lnN " );
            fprintf(fp, " - - - %g - \n",alpha);

        }
        else{
            fprintf(fp, "BwjgmN gmN 0 - - - %g -\n", alpha);
        }
            
        
               // total=0;
                alpha=1.0;
                //for(int i=0; i<totalbins; ++i)total=total+(ttbar[i]);
        
                float ttEC=int(ttbar[Limitbin]+0.5);
                alpha=1.0;
        if(ttEC>0.0000000001){
            alpha=ttbar[Limitbin]/ttEC;
            fprintf(fp, "deltaBttbar lnN  ");
            fprintf(fp, " - - - - %g \n",alpha);
        }
        else {fprintf(fp, "BttgmN gmN 0 - - - - %g \n", alpha);}

   
        
        //}
        float zEC=int(zi[Limitbin]+0.5);
        if(zEC<0.0000001){
            alpha=ttbar[Limitbin]/ttEC;
             fprintf(fp, "BZ gmN 0  - - %g - - \n ", 1.0);
        }
            fprintf(fp, "deltaBZinv lnN ");
            fprintf(fp, " - - %g - - \n",1.25);
        ...
        int nbins=72;
        TString str=(TString::Format("datfiles/SensStudySys_M%d_m%d_%s_Lumi%d.dat", mGlu, MLSP,Options.Data(), lumi));
      ...
      //  const char outputfilename[100]=str.Data();
        ...
        fprintf(fp, "------------\n");
        fprintf(fp, "bin ");
        for(int i=0; i<totalbins; ++i)fprintf(fp, " %d", i);
        
        fprintf(fp, "\n observation ");
        for(int i=0; i<totalbins; ++i)fprintf(fp, "%d ",obs[i]);
...
//NUISSANCES!
            //SUM the total background
            //float total=0;
           // for(int i=0; i<totalbins; ++i)//total=total+(qcd[i]);
        
        
        
        for(int i=0; i<totalbins; ++i){
            float qcdEC=int(qcd[i]+0.5);
            if(qcdEC>0.000001)continue;
            
            fprintf(fp, "deltaBqcd%d gmN 0 ",i );
            //    std::cout<<"qcd "<<qcd[i]<<"sqrt"<<sqrt(qcdEC/10)<<std::endl;
            for(int j=0; j<totalbins; ++j){
                float alpha=1.0;
                if(i==j)fprintf(fp, " - %g - - - ",alpha);
                else fprintf(fp, " - - - - - ");
            }
            fprintf(fp, "\n");
        }
           ...
           for(int i=0; i<totalbins; ++i){
                       float Q=sig[i]/sqrt(sig[i]+qcd[i]+ zi[i]+wj[i]+ttbar[i]);
              // if(Q<0.2)continue;
               float qcdEC=int(qcd[i]+0.5);
               //if(i!=0)continue;
               fprintf(fp, "Bqcd%d lnN ",i );
                        //    std::cout<<"qcd "<<qcd[i]<<"sqrt"<<sqrt(qcdEC/10)<<std::endl;
            for(int j=0; j<totalbins; ++j){
                float alpha=0.0;
                if(qcdEC>0.000001)alpha=sqrt(qcdEC/10.)/qcdEC+1;
                   if(i==j)fprintf(fp, " - %g - - - ",alpha);
                   else fprintf(fp, " - - - - - ");
               }
                fprintf(fp, "\n");
        ...
        for(int i=0; i<totalbins; ++i){
            float wEC=int(wj[i]+0.5);
            if(wEC>0.000001)continue;
            
            fprintf(fp, "Bwj%d gmN 0 ",i );
            //    std::cout<<"qcd "<<qcd[i]<<"sqrt"<<sqrt(qcdEC/10)<<std::endl;
            for(int j=0; j<totalbins; ++j){
                float alpha=1.0;
                if(i==j)fprintf(fp, " - - - %g - ",alpha);
        ...
        for(int i=0; i<totalbins; ++i){
         ...
         //   if(Q<0.2)continue;
            ...
            float wEC=int(wj[i]+0.5);
            //if(i!=0)continue;
            fprintf(fp, "deltaBW%d lnN ",i );
            
            for(int j=0; j<totalbins; ++j){
                float alpha=0.0;
                if(wEC>0.000001)alpha=sqrt(wEC)/wEC +1;
                if(i==j)fprintf(fp, " - - - %g - ",alpha);
                else fprintf(fp, " - - - - - ");
            }
            fprintf(fp, "\n");
        ...
        for(int i=0; i<totalbins; ++i){
            float ttEC=int(ttbar[i]+0.5);
            if(ttEC>0.000001)continue;
            
            fprintf(fp, "Btt%d gmN 0 ",i );
            //    std::cout<<"qcd "<<qcd[i]<<"sqrt"<<sqrt(qcdEC/10)<<std::endl;
            for(int j=0; j<totalbins; ++j){
                float alpha=1.0;
                if(i==j)fprintf(fp, " - - - - %g ",alpha);
        ...
        //    if(Q<0.2)continue;
            ...
            float ttEC=int(ttbar[i]+0.5);
            //if(i!=0)continue;
            fprintf(fp, "deltaBTTbar%d lnN ",i );
            
            for(int j=0; j<totalbins; ++j){
                float alpha=0.0;
                if(ttEC>0.000001)alpha=sqrt(ttEC)/ttEC+1;
                if(i==j)fprintf(fp, " - - - - %g ",alpha);
                else fprintf(fp, " - - - - - ");
            }
            fprintf(fp, "\n");
        ...
        for(int i=0; i<totalbins; ++i){
            float zEC=int(zi[i]+0.5);
            if(zEC>0.000001)continue;
            
            fprintf(fp, "BZ%d gmN 0 ",i );
            //    std::cout<<"qcd "<<qcd[i]<<"sqrt"<<sqrt(qcdEC/10)<<std::endl;
            for(int j=0; j<totalbins; ++j){
                float alpha=1.0;
                if(i==j)fprintf(fp, " - -  %g - -",alpha);
        ...
        fprintf(fp, "\n");
        fprintf(fp, "deltaBZin lnN ");
        
        for(int i=0; i<totalbins; ++i){
            
            fprintf(fp, " - - %1.2f - - ",1.25);
        }
 /*
        fprintf(fp, "\n");
 
        fprintf(fp, "deltaS lnN ");
        ...
        fprintf(fp, "\n");
        fprintf(fp, "deltaB lnN ");
        
        for(int i=0; i<totalbins; ++i){
            fprintf(fp, "- %1.2f %1.2f %1.2f %1.2f ",1.35,1.35,1.35,1.35);
        }
   */
...
sprintf(weightstring, "*(weight*7000)"); //bkg xsec weight
...
TFile*f0=new TFile(TString::Format("Bins4DBINS7fb.root").Data(), "RECREATE");
for(int b=0; b<nBinsBjets; ++b){
